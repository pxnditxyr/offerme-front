...
import { AuthService, ManagementCompanyUsersService, ManagementPromotionRequestsService, ManagementPromotionStatusService, SubparametersService } from '~/services'
...
export const useGetSubparameters = routeLoader$<IGetSubparametersResponse>( async ({ cookie, redirect }) => {
  ...
  const company = await ManagementCompanyUsersService.companyByUserId({ jwt: jwt.value, userId: authResponse.user.id })
  if ( 'errors' in company ) {
    return {
      companies: { errors: 'Unauthorized' },
      promotionTypes: { errors: 'Unauthorized' },
      currencies: { errors: 'Unauthorized' }
    }
  }
...
export const createPromotionRequestAction = routeAction$( async ( data, { cookie, fail } ) => {
  ...
  const promotionRequest = await ManagementPromotionRequestsService.createPromotionRequest({ createPromotionRequestInput: {
    ...data,
