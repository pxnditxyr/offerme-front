...
export const graphqlExceptionsHandler = ( error : any ) : string => {
  ...
  if ( ( error.response.errors.length === 1 ) && ( error.response.errors[ 0 ].extensions.originalError ) ) {
    const errors = ( error.response.errors as IGraphQLValidationError[] ).map( ( error ) => `${ error.extensions.originalError.message }` )
    return errors.join( '\n' )
  }
  const errors = ( error.response.errors as IGraphQLErrorInfo[] ).map( ( error ) => `${ error.message }` )
  return errors.join( '\n' )
}
